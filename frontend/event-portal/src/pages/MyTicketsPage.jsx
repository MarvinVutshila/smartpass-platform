...
export default function MyTicketsPage() {
  ...
  useEffect(() => {
    API.get("/tickets/me")
      .then((r) => {
        // Robust extraction: handle array, { tickets }, { data }, single object, etc.
        let ticketList = [];

        if (Array.isArray(r.data)) {
          ticketList = r.data;
        } else if (r.data && typeof r.data === "object") {
          const possibleKeys = ["tickets", "data", "results", "items", "list", "ticketList"];
          for (const key of possibleKeys) {
            if (r.data[key] && Array.isArray(r.data[key])) {
              ticketList = r.data[key];
              break;
            }
          }
          // If still empty and the object itself looks like a ticket, wrap it
          if (ticketList.length === 0 && r.data.id) {
            ticketList = [r.data];
          }
        }
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-emerald-600">
              Ticket #{successId} is now in your wallet below.
            </p>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex items-center justify-between pt-4 border-t border-slate-100">
                    <span className="text-xs text-slate-400">
                      Ticket ID: {t.public_ticket_id?.slice(0, 8) || t.id}
                    </span>
                    <button
          ...
          >
            ...
            >
              <h3 className="font-bold text-slate-900 text-lg mb-1">
                {selected.event_name || `Ticket #${selected.id}`}
              </h3>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex gap-3">
                  <button onClick={() => setSelected(null)} className="btn-outline flex-1 justify-center">
                    Close
                  </button>
                  <a
