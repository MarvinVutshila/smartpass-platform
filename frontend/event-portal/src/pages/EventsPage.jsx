...
export default function EventsPage() {
  ...
  const openModal = async (event) => {
    ...
    setImagePreview(event.image_url ? event.image_url : null);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (selectedEvent) {
        const updatedEvent = { ...selectedEvent, image_url: res.data.url };
        await API.put(`/admin/events/${selectedEvent.id}`, updatedEvent);
      ...
      setImagePreview(selectedEvent?.image_url || null);
    } finally {
      setUploadingImage(false);
    }
  };

  // ─── Create a test ticket for the event ──────────────────────
  const createTestTicket = async (eventId, tierName) => {
    if (!tierName) {
      alert("No ticket type available for this event.");
      return;
    }
    ...
    try {
      ...
      alert("Failed to create test ticket: " + (err.response?.data?.detail || err.message));
    } finally {
      setTestLoading(null);
    }
  };
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
          {events.map((ev, i) => {
            ...
            const lowestPrice = eventTiers.length > 0
              ...
              : (ev.ticket_price || 0);
            const firstTierName = eventTiers.length > 0 ? eventTiers[0].name : "General Admission";
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {/* ─── Test Ticket Button ──────────────── */}
                      <button
                        onClick={() => createTestTicket(ev.id, firstTierName)}
                        disabled={testLoading === ev.id}
                      ...
                      >
                        {testLoading === ev.id ? "Creating…" : "Test Ticket"}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            >
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {selectedEvent.image_url && !imagePreview && (
                    <div className="text-xs text-emerald-600">✓ Image set</div>
                  )}
                </div>
              </div>
